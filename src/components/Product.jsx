import Count from "./Count";

const Product = ({ product, increase, decrease, selected, onSelectChange, toggleLike, isLiked, addToCart }) => {

    // РАСЧЕТ ЦЕНЫ
    const count = product.count || 0;
    const totalPrice = count > 0
    // Если товара в корзине больше 0, она умножает цену на количество.
        ? (product.priceNum * count).toFixed(2)
        // Если товар еще не выбран — просто показывает цену за одну штуку.
        : product.priceNum.toFixed(2);

    return (
        <div className='product__card'>
            {onSelectChange && (
                <input
                    className="product-checkbox"
                    type="checkbox"
                    checked={selected}
                    onChange={(e) => onSelectChange(e.target.checked)}
                />
            )}
            {isLiked && (
                <svg className="favorite-icon" width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M8.93798 16.5C8.66199 16.4997 8.39249 16.4163 8.16454 16.2607C4.78763 13.9683 3.3254 12.3965 2.51888 11.4138C0.800126 9.31906 -0.022726 7.16848 0.000477133 4.83957C0.0275475 2.17078 2.16868 0 4.77345 0C6.66751 0 7.97935 1.06691 8.74333 1.95551C8.76753 1.98337 8.79743 2.00572 8.83101 2.02103C8.8646 2.03634 8.90107 2.04427 8.93798 2.04427C8.97489 2.04427 9.01136 2.03634 9.04494 2.02103C9.07853 2.00572 9.10843 1.98337 9.13263 1.95551C9.89661 1.06605 11.2084 0 13.1025 0C15.7073 0 17.8484 2.17078 17.8755 4.84C17.8987 7.16934 17.075 9.31992 15.3571 11.4142C14.5506 12.3969 13.0883 13.9687 9.71142 16.2611C9.48342 16.4165 9.21392 16.4998 8.93798 16.5Z" fill="#DEAD6F" />
                </svg>

            )}
            <div className='products-image'>
                <img src={product.img} alt={product.title} />
            </div>
            <h3 className='products-title'>{product.title}</h3>
            <div className="star">
                <img src="../../public/img/star.svg" alt="Оценка" />
                <p className='products-star'>{product.star}</p>
            </div>

            <p className='products-price'>${totalPrice}</p>
            {count > 0 ? (
                <Count
                    count={count}
                    increase={increase}
                    decrease={decrease}
                    toggleLike={() => toggleLike(product.index)}
                    isLiked={isLiked}
                />
            ) : (
                <div className="product__btns">
                    <button className="product__btns-cart" onClick={() => addToCart(product.index)}>
                        Add to cart
                    </button>
                    <button className="product__button" onClick={() => toggleLike(product.index)}>
                        {isLiked ? '❤️' : '♡'}
                    </button>
                </div>
            )}
        </div>
    );
};

export default Product;
